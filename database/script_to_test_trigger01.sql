DROP FUNCTION IF EXISTS compute_product_name_tsvector() CASCADE;
CREATE OR REPLACE FUNCTION compute_product_name_tsvector()
RETURNS TRIGGER AS $$
BEGIN
	NEW.name_tsvector := to_tsvector(NEW.name);
	return NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS product_name_tsvector ON product;
CREATE TRIGGER product_name_tsvector 
AFTER INSERT OR UPDATE 
ON product
FOR EACH ROW 
EXECUTE PROCEDURE compute_product_name_tsvector();
